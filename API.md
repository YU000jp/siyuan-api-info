...
Blocks are the fundamental building units of content in SiYuan. Every piece of content (paragraphs, headings, lists, code blocks, etc.) is represented as a block. The Blocks API provides comprehensive control over content creation and manipulation.

### Insert blocks

Inserts a new block at a specific position relative to existing blocks. This is the most flexible way to add content to your documents.
...
- `dataType` (string): Format of the data being inserted
  - `"markdown"`: Markdown format (recommended for most use cases)
  - `"dom"`: Direct DOM/HTML format
- `data` (string): Content to be inserted
- `nextID` (string): ID of the block to insert before (optional)
- `previousID` (string): ID of the block to insert after (optional)
- `parentID` (string): ID of the parent block to insert into (optional)

**Positioning Priority:** `nextID` > `previousID` > `parentID` (at least one must be provided)
...
**Response Fields:**
- `action.data`: Generated DOM for the newly inserted block
- `action.id`: ID of the newly inserted block
- `doOperations`: Array of operations performed
- `undoOperations`: Operations needed to undo this action (null if not available)

**Example - Insert a formatted paragraph:**
```javascript
const insertFormattedBlock = async (content, afterBlockId, token) => {
  const response = await fetch('http://127.0.0.1:6806/api/block/insertBlock', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Token ${token}`
  ...
  const result = await response.json();
  return result.data[0].doOperations[0].id; // Return new block ID
};

// Usage
const newBlockId = await insertFormattedBlock(
  '## New Section\n\nThis is a **bold** statement with *emphasis*.',
  '20211229114650-vrek5x6',
  'your_token'
);
```
...
Inserts a new block as the first child of a parent block.
...
- `dataType` (string): Format of the data (`"markdown"` or `"dom"`)
- `data` (string): Content to be inserted
...
**Example - Add introduction to a section:**
```python
def add_section_intro(parent_block_id, intro_text, token):
    """Add an introduction paragraph at the beginning of a section"""
    import requests
    
    url = "http://127.0.0.1:6806/api/block/prependBlock"
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Token {token}"
    }
...
# Usage
intro = "This section covers the basics of API usage."
new_block = add_section_intro("20220107173950-7f9m1nb", intro, "your_token")
```

**Use Cases:**
- Adding introductions to sections
- Inserting headers or metadata
- Building structured content hierarchies

### Append blocks

Inserts a new block as the last child of a parent block.
...
```javascript
const addTodoItem = async (parentId, todoText, token) => {
  const todoMarkdown = `- [ ] ${todoText}`;
  
  const response = await fetch('http://127.0.0.1:6806/api/block/appendBlock', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Token ${token}`
...
for (const item of todoItems) {
  await addTodoItem("20220107173950-7f9m1nb", item, "your_token");
}
```

**Use Cases:**
- Building lists and collections
- Adding conclusions to sections
- Appending summaries or footnotes
- Dynamic content assembly

### Update a block

Modifies the content of an existing block while preserving its position and relationships.
...
- `dataType` (string): Format of the updated data (`"markdown"` or `"dom"`)
- `data` (string): New content for the block
...
**Response Fields:**
- `action.data`: Generated DOM for the updated block

**Example - Update content with versioning:**
```python
def update_block_content(block_id, new_content, token):
    """Update a block's content and return the operation result"""
    import requests
    ...
    if result["code"] == 0:
        print(f"Successfully updated block {block_id}")
        return result["data"][0]["doOperations"][0]
    else:
        print(f"Failed to update block: {result['msg']}")
        return None

# Usage - Update a status block
update_block_content(
    "20211230161520-querkps",
    "**Status**: Updated on 2024-01-15 - All tests passing ✅",
    "your_token"
)
```
...
- Status updates and live information
- Template-based content updates
- Collaborative editing workflows

### Delete a block

Permanently removes a block and all its child blocks. ⚠️ **This operation cannot be undone.**
...
- `id` (string): ID of the block to be deleted
...
**⚠️ Warning:** This operation permanently deletes the block and all its child blocks. Always backup important content before deletion.

**Example - Safe block deletion with confirmation:**
```javascript
const deleteBlockSafely = async (blockId, token, confirm = false) => {
  if (!confirm) {
    throw new Error('Deletion must be explicitly confirmed');
  }
  
  const response = await fetch('http://127.0.0.1:6806/api/block/deleteBlock', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Token ${token}`
...
// Usage - explicit confirmation required
try {
  await deleteBlockSafely("20211230162439-vtm09qo", "your_token", true);
} catch (error) {
...
Moves a block to a new position within the document structure.
...
- `id` (string): ID of the block to move
- `previousID` (string): ID of the block to position after (optional)
- `parentID` (string): ID of the parent block to move into (optional)

**Note:** `previousID` and `parentID` cannot both be empty, but you can use one or both for positioning.
